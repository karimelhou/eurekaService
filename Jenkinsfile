        ...
        stage('Docker Build') {
            steps {
                script {
                    ...
                    dockerImage = docker.build("eurekaservice-app:${env.BUILD_ID}")
                }
            }
        }
        ...
        stage('Push') {
            steps {
                script {
                    ...
                    sh "docker tag eurekaservice-app:${env.BUILD_ID} karimelhou/eurekaservice:${env.BUILD_ID}"
                    // Pushing the image to Docker Hub
                    sh "docker push karimelhou/eurekaservice:${env.BUILD_ID}"
                }
            }
        }
        ...
        stage('Docker Run') {
            steps {
                script {
                    ...
                    sh "docker run -d -p 8761:8761 eurekaservice-app:${env.BUILD_ID}"
                }
            }
        }
